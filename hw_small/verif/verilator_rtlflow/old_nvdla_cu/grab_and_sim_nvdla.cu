...
public:
	CSBMaster(RF::VNV_nvdla *_dla, size_t idx):rf_dla{_dla}, idx{idx} {
...
int main(int argc, const char **argv, char **env) {
  ...
  size_t gpu_threads = GPU_THREADS;
  ...
  std::vector<TraceLoader*> rf_trace(files.size(), nullptr);
  std::vector<CSBMaster*> rf_csb(files.size(), nullptr);
  std::vector<AXIResponder*> rf_axi_dbb(files.size(), nullptr);
  std::vector<AXIResponder*> rf_axi_cvsram(files.size(), nullptr);
  std::vector<size_t> rf_targeting_tbs(gpu_threads);
  std::iota(rf_targeting_tbs.begin(), rf_targeting_tbs.end(), 0);
  ...
  // grab first GPU_THREADS testbenches
...
////#endif
  ...
  std::vector<bool> need_resets(gpu_threads, true);
  std::vector<size_t> reset_times(gpu_threads, 0);

  printf("reset...\n");
  printf("letting buffers clear after reset...\n");

  for(size_t c = 0; c < 8235; ++c) {
    for(size_t t = 0; t < gpu_threads; ++t) {
      if(need_resets[t]) {
        if(reset_times[t] == 0) { // reset, 21 times
          *(rtlflow.get(rf_dla->dla_reset_rstn, t)) = 1;
          *(rtlflow.get(rf_dla->direct_reset_, t)) = 1;
        }
        else if(reset_times[t] == 21) { // reset, 21 times
          *(rtlflow.get(rf_dla->dla_reset_rstn, t)) = 0;
          *(rtlflow.get(rf_dla->direct_reset_, t)) = 0;
        }
        else if(reset_times[t] == 42) { // clean buffer, 8192 times
          *(rtlflow.get(rf_dla->dla_reset_rstn, t)) = 1;
          *(rtlflow.get(rf_dla->direct_reset_, t)) = 1;
        }
        ...
        if(reset_times[t] == 8234) { // complete
          reset_times[t] = 0; 
          need_resets[t] = false;
        }
      }
    ...
    cudaMemset(rtlflow.get(rf_dla->dla_core_clk, 0), 1, sizeof(unsigned char) * gpu_threads);
    cudaMemset(rtlflow.get(rf_dla->dla_csb_clk, 0), 1, sizeof(unsigned char) * gpu_threads);

    rf_dla->eval();
    ticks++;

    cudaMemset(rtlflow.get(rf_dla->dla_core_clk, 0), 0, sizeof(unsigned char) * gpu_threads);
    cudaMemset(rtlflow.get(rf_dla->dla_csb_clk, 0), 0, sizeof(unsigned char) * gpu_threads);
...
////#endif
  ...
  //std::atomic<size_t> alldone{0};
  std::atomic<size_t> grabbed_tbs{gpu_threads};
  bool alldone{false};
  //size_t grabbed_tbs{gpu_threads};
	std::vector<int> rf_waitings(gpu_threads, 0);
  ...
  while ((!alldone) || (quiesc_timer--)) {
    ...
    for(size_t t = 0; t < gpu_threads; ++t) {
      ...
      if(!rtlflow.done[t]) {
        if(need_resets[t]) {
          ...
          if(reset_times[t] == 8234) { // finish resetting, prepare to simulate
            reset_times[t] = 0; 
            need_resets[t] = false;
          }
        }
        else {
          // targeting testbench simulating...
          size_t target_tb = rf_targeting_tbs[t];

          int rf_extevent;
          int extevent;

          rf_extevent = rf_csb[target_tb]->rf_eval(rf_waitings[t]);

          if (rf_extevent == TraceLoader::TRACE_AXIEVENT)
            rf_trace[target_tb]->axievent();
            ...
            rf_trace[target_tb]->syncpt(rf_extevent);
          }

          if (rf_waitings[t] && *(rtlflow.get(rf_dla->dla_intr, t))) {
            printf("(%lu) interrupt!\n", ticks);
            rf_waitings[t] = 0;
          }

          rf_axi_dbb[target_tb]->eval();

          if (rf_axi_cvsram[target_tb] != nullptr)
            rf_axi_cvsram[target_tb]->eval();
        }
      }
    }
    ...
    #pragma omp parallel for
    for(size_t t = 0; t < gpu_threads; ++t) {
      // check if the current testbench is done
      size_t target_tb = rf_targeting_tbs[t];
      if(rf_csb[target_tb]->done()) {
        if(!rtlflow.done[t]) {
          alldone = false;
          size_t new_target_tb = grabbed_tbs.fetch_add(1);
          if(new_target_tb < files.size()) {
            // grab a new testbench

            rf_waitings[t] = 0;
            need_resets[t] = true;
            rf_csb[new_target_tb] = new CSBMaster(rf_dla, t);
            ...
            rf_axi_dbb[new_target_tb] = new AXIResponder(rf_dbbconn, "DBB");
              ...
              rf_axi_cvsram[new_target_tb] = new AXIResponder(rf_cvsramconn, "CVSRAM");
            #else
              rf_axi_cvsram[new_target_tb] = nullptr;
            #endif

            rf_trace[new_target_tb] = new TraceLoader(rf_csb[new_target_tb], rf_axi_dbb[new_target_tb], rf_axi_cvsram[new_target_tb]);
          ...
          //#endif
            ...
            rf_trace[new_target_tb]->load(files[new_target_tb].c_str());
            rf_targeting_tbs[t] = new_target_tb;
              //trace->load(argv[t + 1]);
          }
          else {
            // there is no new testbench left
            // waiting for unfinished testbench
            rtlflow.done[t] = true;
            //alldone.fetch_add(1);
          }
        }
