...
int main(int argc, const char **argv, char **env) {
  ...
  size_t NUM_TESTBENCHES;
  {
    ...
    sstream >> NUM_TESTBENCHES;
  }

  std::vector<std::string> files;
  files.reserve(NUM_TESTBENCHES);
  std::cerr << "file\n";

  std::string s(argv[1]);
  s += "/tb";
  for(size_t i = 1; i <= 16384; ++i) {
    std::ifstream ifile(s + std::to_string(i));
    if(ifile.is_open()) {
      std::string tb;
      while(std::getline(ifile, tb) && (files.size() < NUM_TESTBENCHES)) {
        files.emplace_back("../../../verif/verilator/traces/" + tb + "/trace.bin");
      }
    }
    if(files.size() == NUM_TESTBENCHES) {
      break;
    }
  }

  const size_t NUM_BATCHES = NUM_TESTBENCHES / GPU_THREADS;
  std::cerr << "batch\n" << NUM_BATCHES;

  std::vector<RF::RTLflow*> rtlflows;
  //if(GPU_THREADS > files.size()) {
    //GPU_THREADS = files.size();
    //std::memset(rtlflow.done + GPU_THREADS, 1, sizeof(bool) * (GPU_THREADS - files.size()));
  //}
  ...
  std::cerr << "flowsh\n";
	...
	std::vector<RF::VNV_nvdla*> rf_dlas(NUM_BATCHES, nullptr);
  for(size_t i = 0; i < NUM_BATCHES; ++i) {
    rtlflows[i] = new RF::RTLflow(GPU_THREADS);
    rf_dlas[i] = new RF::VNV_nvdla(*(rtlflows[i]));
  }
  std::cerr << "11111\n";
  ...
  //std::vector<size_t> rf_working_tbs(GPU_THREADS);
  //std::iota(rf_working_tbs.begin(), rf_working_tbs.end(), 0);

  //std::vector<TraceLoader*> rf_work_trace(GPU_THREADS, nullptr);
  //std::vector<CSBMaster*> rf_work_csb(GPU_THREADS, nullptr);
  //std::vector<AXIResponder*> rf_work_axi_dbb(GPU_THREADS, nullptr);
  //std::vector<AXIResponder*> rf_work_axi_cvsram(GPU_THREADS, nullptr);
  ...
  for(size_t b = 0; b < NUM_BATCHES; ++b) {
    for(size_t t = 0; t < GPU_THREADS; ++t) {
      size_t id = b * GPU_THREADS + t;
      rf_csb[id] = new CSBMaster(*rtlflows[b], rf_dlas[b], t);
      
      AXIResponder::connections rf_dbbconn = {
        .aw_awvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_aw_awvalid, t),
        .aw_awready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_aw_awready, t),

        .aw_awid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_aw_awid, t),
        .aw_awlen = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_aw_awlen, t),
        .aw_awaddr = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_aw_awaddr, t),
        
        .w_wvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_w_wvalid, t),
        .w_wready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_w_wready, t),
        .w_wdata = AXI_WDATA_MKPTR *(rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_w_wdata, t)),
        .w_wstrb = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_w_wstrb, t),
        .w_wlast = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_w_wlast, t),
        
        .b_bvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_b_bvalid, t),
        .b_bready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_b_bready, t),
        .b_bid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_b_bid, t),

        .ar_arvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_ar_arvalid, t),
        .ar_arready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_ar_arready, t),
        .ar_arid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_ar_arid, t),
        .ar_arlen = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_ar_arlen, t),
        .ar_araddr = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_ar_araddr, t),
      
        .r_rvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_r_rvalid, t),
        .r_rready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_r_rready, t),
        .r_rid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_r_rid, t),
        .r_rlast = rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_r_rlast, t),
        .r_rdata = AXI_WDATA_MKPTR *(rtlflows[b]->get(rf_dlas[b]->nvdla_core2dbb_r_rdata, t)),
      };
      rf_axi_dbb[id] = new AXIResponder(rf_dbbconn, "DBB");

    #ifdef NVDLA_SECONDARY_MEMIF_ENABLE
      AXIResponder::connections rf_cvsramconn = {
        .aw_awvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_aw_awvalid, t),
        .aw_awready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_aw_awready, t),
        .aw_awid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_aw_awid, t),
        .aw_awlen = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_aw_awlen, t),
        .aw_awaddr = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_aw_awaddr, t),
        
        .w_wvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_w_wvalid, t),
        .w_wready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_w_wready, t),
        .w_wdata = *(rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_w_wdata, t)),
        .w_wstrb = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_w_wstrb, t),
        .w_wlast = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_w_wlast, t),
        
        .b_bvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_b_bvalid, t),
        .b_bready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_b_bready, t),
        .b_bid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_b_bid, t),

        .ar_arvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_ar_arvalid, t),
        .ar_arready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_ar_arready, t),
        .ar_arid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_ar_arid, t),
        .ar_arlen = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_ar_arlen, t),
        .ar_araddr = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_ar_araddr, t),
      
        .r_rvalid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_r_rvalid, t),
        .r_rready = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_r_rready, t),
        .r_rid = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_r_rid, t),
        .r_rlast = rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_r_rlast, t),
        .r_rdata = *(rtlflows[b]->get(rf_dlas[b]->nvdla_core2cvsram_r_rdata, t)),
      };
      rf_axi_cvsram[id] = new AXIResponder(rf_cvsramconn, "CVSRAM");
      #else
        rf_axi_cvsram[id] = nullptr;
      #endif

      rf_trace[id] = new TraceLoader(rf_csb[id], rf_axi_dbb[id], rf_axi_cvsram[id]);

      *(rtlflows[b]->get(rf_dlas[b]->global_clk_ovr_on, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->tmc2slcg_disable_clock_gating, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->test_mode, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->nvdla_pwrbus_ram_c_pd, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->nvdla_pwrbus_ram_ma_pd, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->nvdla_pwrbus_ram_mb_pd , t))= 0;
      *(rtlflows[b]->get(rf_dlas[b]->nvdla_pwrbus_ram_p_pd, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->nvdla_pwrbus_ram_o_pd, t)) = 0;
      *(rtlflows[b]->get(rf_dlas[b]->nvdla_pwrbus_ram_a_pd, t)) = 0;

      rf_trace[id]->load(files[id].c_str());
      //trace->load(argv[t + 1]);
    }
  }
  std::cerr << "HIHIHIH\n";
  ...
  printf("reset...\n");
  for(size_t b = 0; b < NUM_BATCHES; ++b) {
    cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_reset_rstn, 0), 1, sizeof(unsigned char) * GPU_THREADS);
    cudaMemset(rtlflows[b]->get(rf_dlas[b]->direct_reset_, 0), 1, sizeof(unsigned char) * GPU_THREADS);
    rf_dlas[b]->eval();
  }
  //dla->eval();
  ...
  for (int i = 0; i < 20; i++) {
    for(size_t b = 0; b < NUM_BATCHES; ++b) {
      cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_core_clk, 0), 1, sizeof(unsigned char) * GPU_THREADS);
      cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_csb_clk, 0), 1, sizeof(unsigned char) * GPU_THREADS);
      rf_dlas[b]->eval();
      ticks++;
    
      cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_core_clk, 0), 0, sizeof(unsigned char) * GPU_THREADS);
      cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_csb_clk, 0), 0, sizeof(unsigned char) * GPU_THREADS);
      rf_dlas[b]->eval();
      ticks++;
    }
  }
  ...
  for(size_t b = 0; b < NUM_BATCHES; ++b) {
    cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_reset_rstn, 0), 0, sizeof(unsigned char) * GPU_THREADS);
    cudaMemset(rtlflows[b]->get(rf_dlas[b]->direct_reset_, 0), 0, sizeof(unsigned char) * GPU_THREADS);
    rf_dlas[b]->eval();
  }
  ...
  for (int i = 0; i < 8192; i++) {
	...
	std::vector<uint32_t> quiesc_timers(NUM_BATCHES, 200);

  tf::Taskflow taskflow;
  tf::Executor executor;
	std::vector<int> rf_waitings(NUM_TESTBENCHES, 0);

  std::vector<tf::Task> cpus(NUM_BATCHES);
  std::vector<tf::Task> gpus(NUM_BATCHES);
  std::vector<tf::Task> check_dones(NUM_BATCHES);
  tf::Task start = taskflow.emplace([](){}).name("start");
  tf::Task end = taskflow.emplace([](){}).name("end");
  std::cerr << "HIII\n";

  for(size_t i = 0; i < NUM_BATCHES; ++i) {
    // cpus set inputs...
    cpus[i] = taskflow.emplace([&, i](){

      #pragma omp parallel for
      for(size_t t = i * GPU_THREADS; t < (i + 1) * GPU_THREADS; ++t) {
        int rf_extevent;
        int extevent;
        rf_extevent = rf_csb[t]->rf_eval(rf_waitings[t]);
        ...
        if (rf_waitings[t] && *(rtlflows[i]->get(rf_dlas[i]->dla_intr, t))) {
          printf("(%lu) interrupt!\n", ticks);
          rf_waitings[t] = 0;
        }
    ...
    gpus[i] = taskflow.emplace([&, i](){
      // gpus perform simulation...
      for(size_t b = 0; b < NUM_BATCHES; ++b) {
        cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_core_clk, 0), 1, sizeof(unsigned char) * GPU_THREADS);
        cudaMemset(rtlflows[b]->get(rf_dlas[b]->dla_csb_clk, 0), 1, sizeof(unsigned char) * GPU_THREADS);
        rf_dlas[b]->eval();
    ...
    // check if all testbenches are done
    check_dones[i] = taskflow.emplace([&, i](){
      bool alldone = true;
      for(size_t t = i * GPU_THREADS; t < (i + 1) * GPU_THREADS; ++t) {
        if(rf_csb[t]->done()) {
          rtlflows[i]->done[t - i * GPU_THREADS] = true;
        }
        else {
          alldone = false;
        }
      }
      return alldone || (!(quiesc_timers[i]--));
    });

    start.precede(check_dones[i]);
    check_dones[i].precede(cpus[i], end);
    cpus[i].precede(gpus[i]);
    gpus[i].precede(check_dones[i]);
  }

  //for(size_t i = 0; i < NUM_BATCHES - 1; ++i) {
    //gpus[i].precede(gpus[i+1]);
  //}
