...
public:
	CSBMaster(RF::VNV_nvdla *_dla, size_t idx):dla{_dla}, idx{idx} {
		
		*(rtlflow.get(dla->csb2nvdla_valid, idx)) = 0;
		_test_passed = 1;
		quiet = 0;
	}
	...
	int eval(int noop) {
		if (*(rtlflow.get(dla->nvdla2csb_wr_complete, idx)))
    ...
    *(rtlflow.get(dla->csb2nvdla_valid, idx)) = 0;
		...
		if (!op.write && op.reading && *(rtlflow.get(dla->nvdla2csb_valid, idx))) {
			if (!quiet) printf("(%lu) read response from nvdla: %08x\n", ticks, *(rtlflow.get(dla->nvdla2csb_data, idx)));
			
			if (op.why == IS_MASK) {
				last_mask = *(rtlflow.get(dla->nvdla2csb_data, idx));
				opq.pop();
			} else if (op.why == IS_STATUS) {
				last_status = *(rtlflow.get(dla->nvdla2csb_data, idx));
				opq.pop();
			...
			} else if ((*(rtlflow.get(dla->nvdla2csb_data, idx)) & op.mask) != (op.data & op.mask)) {
				op.reading = 0;
				op.tries--;
		...
		if (!*(rtlflow.get(dla->csb2nvdla_ready, idx))) {
			if (!quiet) printf("(%lu) CSB stalled...\n", ticks);
			return 0;
		}
		
		if (op.write) {
			*(rtlflow.get(dla->csb2nvdla_valid, idx)) = 1;
			*(rtlflow.get(dla->csb2nvdla_addr, idx)) = op.addr;
			*(rtlflow.get(dla->csb2nvdla_wdat, idx)) = op.data;
			*(rtlflow.get(dla->csb2nvdla_write, idx)) = 1;
			*(rtlflow.get(dla->csb2nvdla_nposted, idx)) = 0;
			if (!quiet) printf("(%lu) write to nvdla: addr %08x, data %08x\n", ticks, op.addr, op.data);
			opq.pop();
		} else {
			*(rtlflow.get(dla->csb2nvdla_valid, idx)) = 1;
			*(rtlflow.get(dla->csb2nvdla_addr, idx)) = op.addr;
			*(rtlflow.get(dla->csb2nvdla_write, idx)) = 0;
...
int main(int argc, const char **argv, char **env) {
	RF::VNV_nvdla *dla = new RF::VNV_nvdla;
  std::vector<TraceLoader*> trace(NUM_TESTBENCHES, nullptr);
  std::vector<CSBMaster*> csb(NUM_TESTBENCHES, nullptr);
  std::vector<AXIResponder*> axi_dbb(NUM_TESTBENCHES, nullptr);
  std::vector<AXIResponder*> axi_cvsram(NUM_TESTBENCHES, nullptr);
  ...
  for(size_t t = 0; t < NUM_TESTBENCHES; ++t) {
    csb[t] = new CSBMaster(dla, t);
    
    AXIResponder::connections dbbconn = {
      .aw_awvalid = rtlflow.get(dla->nvdla_core2dbb_aw_awvalid, t),
      .aw_awready = rtlflow.get(dla->nvdla_core2dbb_aw_awready, t),

      .aw_awid = rtlflow.get(dla->nvdla_core2dbb_aw_awid, t),
      .aw_awlen = rtlflow.get(dla->nvdla_core2dbb_aw_awlen, t),
      .aw_awaddr = rtlflow.get(dla->nvdla_core2dbb_aw_awaddr, t),
      
      .w_wvalid = rtlflow.get(dla->nvdla_core2dbb_w_wvalid, t),
      .w_wready = rtlflow.get(dla->nvdla_core2dbb_w_wready, t),
      .w_wdata = AXI_WDATA_MKPTR *(rtlflow.get(dla->nvdla_core2dbb_w_wdata, t)),
      .w_wstrb = rtlflow.get(dla->nvdla_core2dbb_w_wstrb, t),
      .w_wlast = rtlflow.get(dla->nvdla_core2dbb_w_wlast, t),
      
      .b_bvalid = rtlflow.get(dla->nvdla_core2dbb_b_bvalid, t),
      .b_bready = rtlflow.get(dla->nvdla_core2dbb_b_bready, t),
      .b_bid = rtlflow.get(dla->nvdla_core2dbb_b_bid, t),

      .ar_arvalid = rtlflow.get(dla->nvdla_core2dbb_ar_arvalid, t),
      .ar_arready = rtlflow.get(dla->nvdla_core2dbb_ar_arready, t),
      .ar_arid = rtlflow.get(dla->nvdla_core2dbb_ar_arid, t),
      .ar_arlen = rtlflow.get(dla->nvdla_core2dbb_ar_arlen, t),
      .ar_araddr = rtlflow.get(dla->nvdla_core2dbb_ar_araddr, t),
    
      .r_rvalid = rtlflow.get(dla->nvdla_core2dbb_r_rvalid, t),
      .r_rready = rtlflow.get(dla->nvdla_core2dbb_r_rready, t),
      .r_rid = rtlflow.get(dla->nvdla_core2dbb_r_rid, t),
      .r_rlast = rtlflow.get(dla->nvdla_core2dbb_r_rlast, t),
      .r_rdata = AXI_WDATA_MKPTR *(rtlflow.get(dla->nvdla_core2dbb_r_rdata, t)),
    };
    axi_dbb[t] = new AXIResponder(dbbconn, "DBB");

  #ifdef NVDLA_SECONDARY_MEMIF_ENABLE
    AXIResponder::connections cvsramconn = {
      .aw_awvalid = rtlflow.get(dla->nvdla_core2cvsram_aw_awvalid, t),
      .aw_awready = rtlflow.get(dla->nvdla_core2cvsram_aw_awready, t),
      .aw_awid = rtlflow.get(dla->nvdla_core2cvsram_aw_awid, t),
      .aw_awlen = rtlflow.get(dla->nvdla_core2cvsram_aw_awlen, t),
      .aw_awaddr = rtlflow.get(dla->nvdla_core2cvsram_aw_awaddr, t),
      
      .w_wvalid = rtlflow.get(dla->nvdla_core2cvsram_w_wvalid, t),
      .w_wready = rtlflow.get(dla->nvdla_core2cvsram_w_wready, t),
      .w_wdata = *(rtlflow.get(dla->nvdla_core2cvsram_w_wdata, t)),
      .w_wstrb = rtlflow.get(dla->nvdla_core2cvsram_w_wstrb, t),
      .w_wlast = rtlflow.get(dla->nvdla_core2cvsram_w_wlast, t),
      
      .b_bvalid = rtlflow.get(dla->nvdla_core2cvsram_b_bvalid, t),
      .b_bready = rtlflow.get(dla->nvdla_core2cvsram_b_bready, t),
      .b_bid = rtlflow.get(dla->nvdla_core2cvsram_b_bid, t),

      .ar_arvalid = rtlflow.get(dla->nvdla_core2cvsram_ar_arvalid, t),
      .ar_arready = rtlflow.get(dla->nvdla_core2cvsram_ar_arready, t),
      .ar_arid = rtlflow.get(dla->nvdla_core2cvsram_ar_arid, t),
      .ar_arlen = rtlflow.get(dla->nvdla_core2cvsram_ar_arlen, t),
      .ar_araddr = rtlflow.get(dla->nvdla_core2cvsram_ar_araddr, t),
    
      .r_rvalid = rtlflow.get(dla->nvdla_core2cvsram_r_rvalid, t),
      .r_rready = rtlflow.get(dla->nvdla_core2cvsram_r_rready, t),
      .r_rid = rtlflow.get(dla->nvdla_core2cvsram_r_rid, t),
      .r_rlast = rtlflow.get(dla->nvdla_core2cvsram_r_rlast, t),
      .r_rdata = *(rtlflow.get(dla->nvdla_core2cvsram_r_rdata, t)),
    };
    axi_cvsram[t] = new AXIResponder(cvsramconn, "CVSRAM");
  #else
    axi_cvsram[t] = nullptr;
  #endif

  trace[t] = new TraceLoader(csb[t], axi_dbb[t], axi_cvsram[t]);
...
//#endif
    ...
    *(rtlflow.get(dla->global_clk_ovr_on, t)) = 0;
    *(rtlflow.get(dla->tmc2slcg_disable_clock_gating, t)) = 0;
    *(rtlflow.get(dla->test_mode, t)) = 0;
    *(rtlflow.get(dla->nvdla_pwrbus_ram_c_pd, t)) = 0;
    *(rtlflow.get(dla->nvdla_pwrbus_ram_ma_pd, t)) = 0;
    *(rtlflow.get(dla->nvdla_pwrbus_ram_mb_pd , t))= 0;
    *(rtlflow.get(dla->nvdla_pwrbus_ram_p_pd, t)) = 0;
    *(rtlflow.get(dla->nvdla_pwrbus_ram_o_pd, t)) = 0;
    *(rtlflow.get(dla->nvdla_pwrbus_ram_a_pd, t)) = 0;

    trace[t]->load(argv[t + 1]);
  }

	printf("reset...\n");
  for(size_t t = 0; t < NUM_TESTBENCHES; ++t) {
    *(rtlflow.get(dla->dla_reset_rstn, t)) = 1;
    *(rtlflow.get(dla->direct_reset_, t)) = 1;
  }
	dla->eval();

	for (int i = 0; i < 20; i++) {
    for(size_t t = 0; t < NUM_TESTBENCHES; ++t) {
      *(rtlflow.get(dla->dla_core_clk, t)) = 1;
      *(rtlflow.get(dla->dla_csb_clk, t)) = 1;
    }
		dla->eval();
		ticks++;
//#if VM_TRACE
...
//#endif
    ...
    for(size_t t = 0; t < NUM_TESTBENCHES; ++t) {
      *(rtlflow.get(dla->dla_core_clk, t)) = 0;
      *(rtlflow.get(dla->dla_csb_clk, t)) = 0;
    }
		dla->eval();
		ticks++;
//#if VM_TRACE
...
//#endif
  ...
  for(size_t t = 0; t < NUM_TESTBENCHES; ++t) {
    *(rtlflow.get(dla->dla_reset_rstn, t)) = 0;
    *(rtlflow.get(dla->direct_reset_, t)) = 0;
  }
	dla->eval();
...
//#endif
  ...
  while (!alldone || (quiesc_timer--)) {
    ...
    for(size_t t = 0; t < NUM_TESTBENCHES; ++t) {
      int extevent;
      extevent = csb[t]->eval(waiting);
      rtlflow.change[t] = !csb[t]->done();
      alldone &= csb[t]->done();
      ...
      if (extevent == TraceLoader::TRACE_AXIEVENT)
        ...
        trace[t]->syncpt(extevent);
      }
      
      if (waiting && *(rtlflow.get(dla->dla_intr, t))) {
        printf("(%lu) interrupt!\n", ticks);
        waiting = 0;
      }
      axi_dbb[t]->eval();
      if (axi_cvsram[t] != nullptr)
        axi_cvsram[t]->eval();
      *(rtlflow.get(dla->dla_core_clk, t)) = 1;
      *(rtlflow.get(dla->dla_csb_clk, t)) = 1;
    }
