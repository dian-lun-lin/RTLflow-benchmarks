...
public:
	CSBMaster(RF::VNV_nvdla *_dut, size_t idx):dut{_dut}, idx{idx} {
		
		*(dut->get(dut->csb2nvdla_valid, idx)) = 0;
		_test_passed = 1;
		quiet = 0;
	}
	...
	int eval(int noop) {
		if (*(dut->get(dut->nvdla2csb_wr_complete, idx)))
    ...
    *(dut->get(dut->csb2nvdla_valid, idx)) = 0;
		...
		if (!op.write && op.reading && *(dut->get(dut->nvdla2csb_valid, idx))) {
			if (!quiet) printf("(%lu) read response from nvdla: %08x\n", ticks, *(dut->get(dut->nvdla2csb_data, idx)));
			
			if (op.why == IS_MASK) {
				last_mask = *(dut->get(dut->nvdla2csb_data, idx));
				opq.pop();
			} else if (op.why == IS_STATUS) {
				last_status = *(dut->get(dut->nvdla2csb_data, idx));
				opq.pop();
			...
			} else if ((*(dut->get(dut->nvdla2csb_data, idx)) & op.mask) != (op.data & op.mask)) {
				op.reading = 0;
				op.tries--;
		...
		if (!*(dut->get(dut->csb2nvdla_ready, idx))) {
			if (!quiet) printf("(%lu) CSB stalled...\n", ticks);
			return 0;
		}
		
		if (op.write) {
			*(dut->get(dut->csb2nvdla_valid, idx)) = 1;
			*(dut->get(dut->csb2nvdla_addr, idx)) = op.addr;
			*(dut->get(dut->csb2nvdla_wdat, idx)) = op.data;
			*(dut->get(dut->csb2nvdla_write, idx)) = 1;
			*(dut->get(dut->csb2nvdla_nposted, idx)) = 0;
			if (!quiet) printf("(%lu) write to nvdla: addr %08x, data %08x\n", ticks, op.addr, op.data);
			opq.pop();
		} else {
			*(dut->get(dut->csb2nvdla_valid, idx)) = 1;
			*(dut->get(dut->csb2nvdla_addr, idx)) = op.addr;
			*(dut->get(dut->csb2nvdla_write, idx)) = 0;
...
int main(int argc, const char **argv, char **env) {
  ...
  std::vector<RF::RTLflow> rtlflows(NUM_PIPES, dut);
  ...
  for(size_t p = 0; p < NUM_PIPES; ++p) {
    for(size_t t = 0; t < RF::THREADS; ++t) {
      csbs[p][t] = new CSBMaster(rtlflows[p], t);
