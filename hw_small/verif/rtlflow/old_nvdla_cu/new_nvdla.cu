...
int main(int argc, const char **argv, char **env) {
  ...
  if (argc != 2) {
    fprintf(stderr, "here we use the same testbench for mesearument\n");
    return 1;
  }
  //// Verilator ========================================================== 
...
//#endif
    ...
    *(rtlflow.get(rf_dla->nvdla_pwrbus_ram_a_pd, t)) = 0;
    ...
    rf_trace[t]->load(argv[1]);
  }


	printf("reset...\n");
  rtlflow.memset(rf_dla->dla_reset_rstn, 1);
  rtlflow.memset(rf_dla->direct_reset_, 1);

	rf_dla->eval();


	for (int i = 0; i < 20; i++) {
    rtlflow.memset(rf_dla->dla_core_clk, 1);
    rtlflow.memset(rf_dla->dla_csb_clk, 1);
		rf_dla->eval();
		ticks++;
		
    rtlflow.memset(rf_dla->dla_core_clk, 0);
    rtlflow.memset(rf_dla->dla_csb_clk, 0);
		rf_dla->eval();
		ticks++;
	}

  rtlflow.memset(rf_dla->dla_reset_rstn, 0);
  rtlflow.memset(rf_dla->direct_reset_, 0);
	rf_dla->eval();
