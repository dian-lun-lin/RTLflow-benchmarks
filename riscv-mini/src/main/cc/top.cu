...
void build_pipes(
  ...
  pipes.emplace_back(tf::PipeType::SERIAL, [&, NUM_LINES](tf::Pipeflow& pf) mutable {
...
//#endif
  ...
  assert(NUM_PIPES > 0);
  build_pipes(pipes, NUM_LINES, NUM_PIPES, rtlflows, mems);
  ...
  //int retcode = rtlflow.get(top->io_host_tohost, b) >> 1;
