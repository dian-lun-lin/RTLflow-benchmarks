...
class cudaRoundRobinCapturing : public cudaCapturingBase {
  ...
  private:
    ...
    void _optimize(cudaGraph& graph);
...
// Function: _optimize 
inline void cudaRoundRobinCapturing::_optimize(cudaGraph& graph) {
  ...
  TF_CHECK_CUDA(cudaStreamSynchronize(streams[0]), "failed to synchronize stream");
